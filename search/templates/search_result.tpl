...
<option value="0" selected="selected">都道府県</option>
    {foreach from=$searchParam.prefecture item=row }
        <option value="{$row.itemVal}" {$row.selected}>{$row.itemName}</option>
    {/foreach}
</select>
<!-- Select -->

<div class="jobselectBt listSB changearrow jsJobCategorySelect"><a href="#">{$searchParam.jobCategoryName}</a></div>
</form>
</div>
...
<div id="wrapper">
        ...
        <div class="row">
            ...
            <div class="col-sm-6 col-md-3" itemscope itemtype="http://schema.org/JobPosting"> <a href="/job/detail/{$row.id}">
                ...
                <div class="moreInfo">
                    ...
                    <div><i class="fa fa-jpy fa-fw"></i><span itemprop="baseSalary">{$row.salary_unit} {$row.salary_min|number_format}円～</span></div>
    ...
    {if !empty($areaLink)}
<!--    <div class="forRbBt"><a href="{$areaLink}">姉妹サイト リゾートバイトナビ でも<br>
    このエリアのお仕事いっぱい掲載中</a>
    </div>-->
    {/if}
    <ul class="pagerBox">
